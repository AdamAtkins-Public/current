...
def r_squared(y, estimated):
    ...
    var_of_err = sum( (y[i] - estimated[i])**2 for i in range(len(y)) )
    var_of_data = sum( (y[i] - est_mean)**2 for i in range(len(y)) )
    return 1 - (var_of_err/var_of_data)
...
def evaluate_models_on_training(x, y, models):
    ...
    Returns:
        None
    """
    best_fit = () #tuple containing (model, R^2)
    #Plot data points as blue dots
    pylab.plot(x, y, 'bo')
    for model in models:
        estimated = np.polyval(model,x)
        rs = r_squared(y, estimated)
        if len(best_fit)==0:
            best_fit = (model, rs)
        elif rs > best_fit[1]:
            best_fit = (model, rs)
    ...
    for index in range(len(x)):
        y_.append(np.polyval(best_fit[0],x[index]))
        x_.append(index + 1961)
    pylab.plot(x_,y_, 'r')
    pylab.xlabel('Date')
    pylab.ylabel('Temperature')
    pylab.title('{0} Degree Model with R^2 {1}'.format(len(best_fit[0])-1, best_fit[1]))
    pylab.show()

### Begining of program
raw_data = Climate('Online_Courses\\MIT_6.00.2x\\data\\data.csv')
